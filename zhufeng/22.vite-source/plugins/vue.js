...
function genStyleCode(descriptor, filename) {
  ...
  if (descriptor.styles.length) {
    descriptor.styles.forEach((style, index) => {
      ...
      const styleRequest =
        path.sep + path.relative(process.cwd(), filename) + query;
      styleCode += `\nimport ${JSON.stringify(styleRequest)}`;
    });
    //'/Users/hanyanxx1/Documents/study/JavaScript/10_构建工具/zhufeng/22.vite-source/src/App.vue?vue&type=style&index=0&lang=css'
    return styleCode;
  }
}
